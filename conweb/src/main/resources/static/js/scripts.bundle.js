var isLocalHost = ['127.0.0.1', 'localhost', '192.168.', '10.'].filter((item, index, array) => item == location.hostname || location.hostname.startsWith(item)).length > 0;
var ws_domain = 'chat-ws.xueliang.org';
var api_domain = 'chat-api.xueliang.org';
var website_domain = ['chat.xueliang.org', 'mlrlt.com'].includes(location.hostname) ? location.hostname : 'chat.xueliang.org';
ws_domain = isLocalHost ? location.hostname + ':8090' : ws_domain;
api_domain = isLocalHost ? location.hostname + ':8091' : api_domain;
website_domain = isLocalHost ? location.hostname + ':8080' : website_domain;
var assets_domain = '//' + website_domain + '/';
var api_url_prefix = location.protocol + '//' + api_domain;
var ws_url = (location.protocol == 'http:' ? 'ws:' : 'wss:') + '//' + ws_domain + '/socketjs';
var upload_url = location.protocol + '//uploader.xueliang.org/server-uploader/uploader/upload/file?appId=dev_test';
// var upload_url = 'http://localhost:8020/server-uploader/uploader/upload/file?appId=dev_test';
"use strict";
...
var KTLayoutChat = function () {
    // Private functions
    var _init = function (element, isMatch, onSend, onOperate) {
        ...
        var lastSendMsgTime = (new Date()).getTime();
        KTUtil.on(element, '.card-footer textarea', 'keydown', function(e) {
            var roomId = KTUtil.attr(KTUtil.parents(e.target, '.modal')[0], 'data-room-id');
            if (e.keyCode == 13 && !e.shiftKey) {
                ...
                if (currentTimeMillis - lastSendMsgTime < 500) {
                    e.preventDefault();
                    return;
                }
                var result = onSend({content: $textarea.value, operate: KTUtil.attr($textarea, 'data-operate-type'), roomId: roomId});
                if (result) {
            ...
            var currentTimeMillis = (new Date()).getTime();
            if (currentTimeMillis - lastKeydownTime > 500) {
                lastKeydownTime = currentTimeMillis;
                onSend({operate: 'TYPING', roomId: roomId});
            } 
        });

        KTUtil.on(element, '.card-footer .chat-operate', 'click', function(e) {
        ...
        // KTUtil.on(element, '.card-footer .chat-operate', 'click', function(e) {
        //     onOperate(e.target.classList);
        // });
    }

    var _handleResetMessageList = function(element) {
    ...
    var _handleOtherMessaging = function(element, message, needCheckRoomId) {
        var textColorClassName = 'text-dark-50';
        if (message.memberGender == 1) {
            textColorClassName = 'text-primary';
        } else if (message.memberGender == 0) {
            textColorClassName = 'text-danger';
        }
        var roomId = KTUtil.attr(element, "data-room-id");
        if (needCheckRoomId && roomId != (message.roomId + '')) {
            return false;
        }
        var messagesEl = KTUtil.find(element, '.messages');
        var scrollEl = KTUtil.find(element, '.scroll');
        ...
        html += '        <span class="text-muted font-size-sm">' + message.createTime + '</span>';
        html += '    </div>';
        html += '</div>';
        html += '<div class="mt-2 rounded p-5 bg-light-success font-weight-bold font-size-lg text-left max-w-800px ' + textColorClassName + '">';
    ...
    var _handeMessaging = function(element, message) {
        var textColorClassName = 'text-dark-50';
        if (message.memberGender == 1) {
            textColorClassName = 'text-primary';
        } else if (message.memberGender == 0) {
        ...
        var roomId = KTUtil.attr(element, "data-room-id");
        if (roomId != null && roomId != (message.roomId + '')) {
            return;
        }
        var messagesEl = KTUtil.find(element, '.messages');
        var scrollEl = KTUtil.find(element, '.scroll');
        ...
        // html += '    </div>';
        html += '</div>';
        html += '<div class="mt-2 rounded p-5 bg-light-primary font-weight-bold font-size-lg max-w-800px ' + textColorClassName + '">' + message.content + '</div>';
    ...
    var _handeSystemMessaging = function(element, message) {
        var messagesEl = KTUtil.find(element, '.messages');
        var scrollEl = KTUtil.find(element, '.scroll');
        // var textarea = KTUtil.find(element, 'textarea');
        ...
        var node = document.createElement("DIV");
        KTUtil.addClass(node, 'd-flex flex-column mb-5 align-items-center');

        var html = '';
        html += '<span class="badge bg-secondary">' + message.content + '</span>';
    ...
    var _chatStatusRedraw = function(element, chatStatus) {
        if (chatStatus == null) {
            return;
        }
        if (element == null) {
        ...
        if (!KTUtil.data(element).get('is-match')) {
            return;
        }
        if (chatStatus != KTUtil.data(element).get()) {
            if (chatStatus == ChatStatusEnum.INIT) {
                KTUtil.findAll(element, '.chat-operate').forEach(function(item, index, nodeList) {
                    var operate = item.dataset.operate;
                    if (operate == 'enter') {
                        KTUtil.removeClass(item, 'd-none');
                    } else {
                        KTUtil.addClass(item, 'd-none');
                    }
                })
            } else if (chatStatus == ChatStatusEnum.WAIT) {
                KTUtil.findAll(element, '.chat-operate').forEach(function(item, index, nodeList) {
                    var operate = item.dataset.operate;
                    if (operate == 'exit') {
                        KTUtil.removeClass(item, 'd-none');
            ...
            }else if (chatStatus == ChatStatusEnum.CHATING) {
                KTUtil.findAll(element, '.chat-operate').forEach(function(item, index, nodeList) {
                    var operate = item.dataset.operate;
                    if (operate == 'send' || operate == 'exit') {
                        KTUtil.removeClass(item, 'd-none');
                    } else {
                        KTUtil.addClass(item, 'd-none');
                    }
            ...
            KTUtil.data(element).set(chatStatus);
        }
    }
    ...
    return {
        ...
        setup: function(element, isMatch, onSend, onOperate) {
            _init(element, isMatch, onSend, onOperate);
        },

        _handleResetMessageList: function(element, message) {
            _handleResetMessageList(element);
        },

        handleSelfMessaging: function(element, message) {
            _chatStatusRedraw(element, message.chatStatus);
            _handeMessaging(element, message);
        },

        handleOtherMessaging: function(element, message) {
            _chatStatusRedraw(element, message.chatStatus);
            return _handleOtherMessaging(element, message, false);
        },

        handleSelfPrivateMessaging: function(element, message) {
            _chatStatusRedraw(element, message.chatStatus);
            _handeMessaging(element, message);
        },

        handleOtherPrivateMessaging: function(element, message) {
            _chatStatusRedraw(element, message.chatStatus);
            return _handleOtherMessaging(element, message, true);
        },

        handleFeedbackMessaging: function(element, message) {
            _chatStatusRedraw(element, message.chatStatus);
            _handeMessaging(element, message);
        },

        handleFeedbackReplyMessaging: function(element, message) {
            _chatStatusRedraw(element, message.chatStatus);
            return _handleOtherMessaging(element, message, false);
        },

        handeSystemMessaging: function(element, message) {
            _chatStatusRedraw(element, message.chatStatus);
            _handeSystemMessaging(element, message);
        },

        chatStatusRedraw: function(element, message) {
            _chatStatusRedraw(element, message.chatStatus);
        }
    };
}();
...
var KTLayoutQuickUser = function() {
    ...
    var _originInitResult;

    var _originInit = function(closeBy, toggleBy) {
        // Private properties
        var _element;
        var _offcanvasObject;
    ...
    return {
        init: function(id, closeBy, toggleBy) {
            _originInitResult = _originInit(closeBy, toggleBy);
            _originInitResult.init(id);
            return _originInitResult;
        },

        getElement: function() {
            return _originInitResult.getElement();
        },

        getOffcanvasObject: function() {
            return _originInitResult.getOffcanvasObject();
        }
    };
}();
